...
def get_release_scans(release_file):
    ...
    with urllib.request.urlopen(release_file) as response:
        # 假设文件是文本文件，我们可以直接迭代它（Python 3会尝试解码它）
        # 或者，如果文件是二进制文件，你需要先调用.read()并解码
        # 但在这里，我们假设它是文本文件
        scans = []
        for scan_line in response:
            # Python 3中，直接迭代response会给我们每行的字节，但我们需要字符串
            # 如果urlopen没有自动解码，你可能需要这样做：scan_line = scan_line.decode('utf-8')
            # 但通常，对于文本文件，urlopen会尝试自动解码
            print("single scan_line:", scan_line.decode('utf-8').rstrip('\n'))  # 显示解码后的行，去除换行符
